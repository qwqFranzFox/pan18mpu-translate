...
随着嵌入式计算和物联网（IoT）的兴起，在保持必需的可预测性的前提下，保证这些系统背后的计算架构稳定而安全显得尤为重要。这些系统的持续增长的应用正驱动着物联网设备标志性的“无处不在的连接”。有了网络连接，随之而来的是它提供给恶意攻击者与系统接触的capability，以及相关的安全问题。同时，可定制地过滤和处理传感器数据的操作，也使得这些系统逐渐远离静态代码的范畴。更进一步，我们注意到这些系统的开发逐渐趋向去中心化。来自不同客户/系统设计师的、在不同安全级别上的代码，必须运行在同一个设备上。这种多租户、#emph[混合关键性]@mixed-criticality
又推动了强隔离。
...
无法找到他们。因此，在VII-D中，我们将研究我们的贪心启发式算法在高效利用内存上的影响。
...
为了提供一种强隔离，我们适配了一个基于capability的安全性@prog-semantics
...
PCTrie节点表示法占用44个字节。Composite中的capability槽位可以是多种大小的，最大的大小（64字节）被用于完全存放PCTrie节点。这使得组件可以避免为每个节点分配（retype、激活，以及管理）分散的内存
...
说到跟作者老师沟通……我觉得现在大学的绩点至上之风还是挺难堪的。我个人比较同意#link("https://zhuanlan.zhihu.com/p/673155133", [《我们的工科教育，问题出在了哪里》])一文中的观点。虽然我对综测的政策已经有所了解，但是当我现场听到科研部分算在综测分的10%素质分里面的时候，我还是没忍住觉得这太荒谬了。就像一个游戏的中的装备/流派，玩家难以拿到的东西不仅不够强力，甚至还有副作用，那只能是被人抛弃了——虽然也有另一个方向，那就是被过度加强，成为版本答案；我想上面的大人制定政策的时候也考虑过。不过也可能有老师会说，本科阶段科研不是重点。但是我觉得这样的话，难道要0基础进入研究生吗？当然我知道绩点和综测分高的同学大概率实力也是差不多强的。
