...
#abstract(
  body: [
    嵌入式系统的深入用户市场产生了对它们的廉价、可预测性、可靠性以及安全性的要求。由于这些系统经常连接网络且运行来各种来源的代码，可靠性和安全性变得尤为重要。为了维持低成本和低功耗。很多嵌入式系统只使用存储极为有限的的微控制器（大体上128KB的SRAM）。不幸的是，这些系统的隔离和保护设施通常缺乏生气
    #footnote[本文中个人没有很好的译法的、拥有特定含义的特定词语，保留原样不做翻译。有个人觉得可翻译但不能完全传达意思的，在脚注中用“Translation:
      xxx”标出。]
    #footnote[Translation: lackluster]，使得有条理的可靠性和安全性保障难以实现。
...
随着嵌入式计算和物联网（IoT）的兴起，在保持必需的可预测性的前提下，保证这些系统背后的计算架构稳定而安全显得尤为重要。这些系统的持续增长的应用正驱动着物联网设备标志性的“无处不在的连接”。有了网络连接，随之而来的是它提供给恶意攻击者与系统接触的capability，以及相关的安全问题。同时，可定制地过滤和处理传感器数据的操作，也使得这些系统逐渐远离静态代码的范畴。更进一步，我们注意到这些系统的开发逐渐趋向去中心化。来自不同客户/系统设计师的、在不同安全级别上的代码，必须运行在同一个设备上。这种多租户、#emph[混合关键性]@mixed-criticality 又推动了强隔离。
...
Buffer）。TLB未命中需要遍历页表，并且对内存访问时间有显著的抖动影响。因此，MPU对于不太需要应用内存灵活性的情况下，尤其是TLB的大小远小于应用所需的页面数量的系统，具有一定价值。在这些情景下，显著更低的内存访问抖动提高了可预测性，使得基于MPU的系统能够从中受益。基于在微控制器中MPU的广泛开发，这篇论文将介绍一个基于Composite
...
Tries）的通用API，能够在推广上述两种硬件保护机制的同时，维持系统的可预测性属性。这个系统将保护域抽象为封装了用户级代码、数据和线程运行的#emph[组件（Component）]。一个组件是建立在一个硬件内存保护上下文和一个capability表@capability 上的轻量级抽象，两者都限制了组件对于系统资源的访问。组件实现了调度、内存映射/访问控制（基于PCTries）和I/O等系统资源管理策略，从而不仅为应用，也为系统服务提供了隔离。


Composite使用了PCTries来支持MPU，提供了微控制器内存隔离的基础。然而，为了提供租户之间、不同来源或安全登记的代码之间的强隔离，或者当错误可以被更多地允许时，CPU和I/O的隔离也是必须的。因此，这篇文章结合了带有分级调度@hires 的PCTrie支持和一个抽象并限制了I/O访问以提供CPU和I/O隔离的组件。这里，系统调度器@temp-cap@pred-inter\和I/O管理器运行在用户级，共同实现了一个虚拟机监视器（VMM,
Virtual Machine Monitor）@nova ，使得虚拟机（VMs, Virtual
Machines）可以可预测地、隔离地运行传统的实时操作系统（RTOS, Real-Time
Operating
System）。据我们所知，这是第一个为基于MPU的微控制器设计的虚拟化系统。

#strong[贡献]。我们从讨论MPU的背景（@II[]）开始，然后集中在这篇论文的贡献：
- 一个通过智能地排列组建内存和潜在的共享映射来实现对MPU有限资源的利用的（@III[]）。
...
== 内存保护单元 <II-B>
Fig. 1：MMU 和
MPU的区别。MMU进行基于页的翻译，用虚线标出；相反地，MPU使用基于域的保护，用竖直的实线标出。RX：只读&可执行；RO：只读；RW：读写。
...
M14k
更加简单，$\| cal(R) \| = 1$到$16$，$S = 1$，$cal(A) = \( a_i mod med 4 = 0 \)$，且$caron(s) = 4$。在本研究中，我们集中在
...
的区域的对齐或颗粒度限制阻止对单独的设备地址区域进行保护是，对它们的访问必须由一个服务管理调节（如@io-virt）。
...
MPU
编程，并简单地查清每个应用程序的代码和数据对连续区域的访问。（这句译法相当抽象。）这些系统，虽然简单，但并不支持动态共享关系，并且使得需要用于共享内存和虚拟化的
MPU 区域布局更加混乱复杂。
...
对于对齐、子区域和大小的限制，规定了所有代码的内存布局，同时系统中的数据也显著影响到每个应用需要多少区域。图2描述了两个不同的应用内存布局和他们的共享内存（为了简单，在这个例子里我们忽略内核和
VMM
组件）。在第一个布局里面，内存被随意地排布，而应用需要比平台能提供的更多的区域（2个）。有些共享区域需要分别的
MPU
区域，而非2的幂次对齐的区域需要多个区域。红色的组件需要4个区域，而剩下的组件需要3个区域。在第二种布局中，内存以一定规则排列，从而减少提供所需的保护的区域的数量。在优化之后，没有任何一个组件使用超过两个区域，绿色的组件只需要一个内存区域。注意到为了实现这种最优化的布局，一段未使用的内存出现在红色和黄色组件之间。这代表区域内部碎片化导致的#emph[开支]，我们将在@VII[] 中继续学习。在当前系统中，设计者需要手动排布内存区域，或者系统镜像足够简单以至于区域位置不重要。
...
#[
  #let collapsable = $c o l l a p s a b l e$
  ...
  #algox(
    ...
    pseudocode(
      ...
      [*while* $| { c in C | mono("num_allocated_regions")(c) gt.eq |cal(R)|-1 }|$ *do* ],
      ind,
      [$collapsable = {c in C | exists_(a in A) mono("is_enabled")(a,c) }$],
      [*if* $|collapsable| = 0$ *then*],
      ind,
      [*return* $N o n e$],
      ded,
      [$c = argmax_(c in collapsable)|mono("num_allocated_regions")(c)|$],
...
展示了这个输入每个组件的内存区域的集合，并确定一个在满足系统MPU限制的前提下实现保护的解的算法。我们将一个#emph[arena]定义为可被一段组件访问的带有特殊访问权限（可读或读写）的内存区域。Arena
包含组件代码和只读数据、可写数据、不同组件之间的共享内存区域，以及对应动态内存分配的系统堆。非常重要的是，这些arena指示了一种为通过运行时请求建立的组件的潜在的映射。

如果所有的组件都有少于$\| cal(R) \|$个arena，那么将arena看作区域没有影响。否则我们尝试将两个区域的arena合并成一个区域，然后用分离的子区域来维持arena之间的隔离。为了有效地利用II-B中引入的MPU，多个arena必须使用一个区域内的分离的子区域，因为这能减少组件所需的区域数量。如果一个组件需要太多的区域，我们必须决定它的哪些区域要和其他区域合并。因此我们选择一个有最多区域的组件（第5行）。在这个组件以内，我们选择和最多组件共享的arena。直觉上，去除对这个区域的需要会减少最大的组件的区域需求。为了寻找另一个可合并的区域（#emph[partner]），我们考虑这个组件的所有区域。然而，可能会出现和另一个区域合并时，所需要的子区域超过$S$的情况，这样必须重新开始搜索（9-11行）。通过为每一个组件-arena对维持一个#emph[enabled]布尔变量，我们确保了算法不会重复合并两个相同的arena。最后，我们选择用于合并的区域，这个区域有最多的使用它作为第一个区域（$a_0$）的组件相交。重复这个过程直到所有区域都能被子区域保护。
...
1输出的解，并且发现了启发式算法次优的几个方面。和启发式算法相比，SMT算法往往：（1）使用一个大区域后面的连续的额外区域，由此产生一个非2的幂次方大小的子区域；（2）将不同大小的区域放进连续的子区域来最小化内部碎片化，以及（3）使得arena能够延伸到多个连续region，并且可能有变化的子区域大小。这些次优性可能指向一些能够适配有限数量的MPU区域/子区域和智能内存布局的内存量的系统配置，但是@algo:算法1 无法找到他们。因此，在VII-D中，我们将研究我们的贪心启发式算法在高效利用内存上的影响。

#strong[内存放置的工作流。]图3展示了每个组件、可能的共享的arena和硬件限制的特性是如何集成到@algo:算法1 中的。的输出被输入给一个生成系统镜像的连接器脚本，以及系统中引导动态请求的内存管理服务。

= 为硬件内存保护推广操作系统支持 <IV>

为了提供一种强隔离，我们适配了一个基于capability的安全性@prog-semantics 的现有的操作系统的访问控制抽象层，而不是一个现有的嵌入式RTOS的特别的接口。capability为基础的系统的核心性质包括：系统资源只能通过#emph[不可伪造的]capability引用，这些capability只在由别的已经拥有这个资源的组件#emph[委派]时才可以访问。所有系统内存和内核资源都通过capability访问，它们也可以用于实现基于#emph[封闭]@confinement 的强隔离。对于一个简单的、注重将大多数系统资源管理转移到用户层的系统，capability已经被验证是很好的@scale-pred @fiasco @sel4 @eros。

这项研究使用一个基于PCTrie的机制来推广基于capability的内存访问控制，这个Composite中的机制——先前是MMU特有的——来控制MPU和MMU。为了简化内存并将动态内核内存分配移动到用户层，Composite使用内存retyping来实现#emph[用户级别]的#emph[内核内存]管理@scale-pred，其中绝大部分由seL4@sel4 启发而来。
...
一个线程是线性执行的基本单位，而每个组件可以有0个或多个线程。一个拥有一个线程的capability的组件能够调度这个线程。通信端点有两种形式：（1）使用线程迁移@mach-thread 提供#emph[同步]通信的`sinv/ret`，以及（2）提供#emph[异步]事件通知的`asnd/rcv`，其中一个线程通过`asnd`#emph[发送]一个事件，激活一个使用`rcv`等待的线程。对资源表自身的操作包括构造和析构资源表（例如，创建一个新的组件），以及从一个资源表复制一个capability到另一个资源表。后者提供了组件之间的#emph[委派]，并且需要一个组件有一个客户的资源表的capability。
...
#strong[路径压缩的基数树（PCTrie）。]与通常的连续几层翻译固定位数地址的页表为基础的基数树相比，谨慎的页表和广义的压缩的基数树@guarded-page 翻译变化位数的地址。这实现了更加内存高效的表示，同时支持发散的地址空间，允许页的大小可以为2的任意次幂。我们系统中的基数树将地址映射到访问权限而不是物理地址；由于MPU并不进行地址翻译，访问权限是足够的。
...
PCTrie节点表示法占用44个字节。Composite中的capability槽位可以是多种大小的，最大的大小（64字节）被用于完全存放PCTrie节点。这使得组件可以避免为每个节点分配（retype、激活，以及管理）分散的内存 @scale-pred。重要的是，这意味着内存管理的开支被均摊到capability表的管理中，而通过移除页颗粒度控制，我们省下了大量的内存。用户层的库管理所有的页表管理细节（使用安全的内核API），这些库被改写以适应PCTrie。

#strong[高效的MPU更新。]ARMv7-M 处理器上的MPU是通过一组内存映射的寄存器编程的，叫做区域编号寄存器（RNR, Region Number Register），区域基址寄存器（RBAR, Region Base Address Register），以及区域属性和大小寄存器（RASR, Region Attribute and Size Register）。RNR控制那个区域正在被编程（尽管RBAR有一个覆盖RNR的区域字段），而RBAR和RASR控制区域的开始地址、区域的大小，以及区域的子区域状态。加载这些寄存器并没有硬件辅助，所以他们必须被手动填充。
...
RBAR/RASR在内存中连续排布，并且紧接着3个别名。基于这个性质，使用ARM Cortex-M7 处理器的多寄存器加载/存储（LDM/STM）指令可以以#emph[每次4个区域]的速度加快MPU编程。VII-A评估了我们对这些指令的应用与制造商提供的硬件抽象层代码的对比。
...
为了实现这些目标，我们使用#emph[半虚拟化]@xen 来避免复杂和模拟原始硬件访问（例如，I/O访问，计时器访问以及中断）。Composite内核至关重要的承担访问监视器的职责——合理地限制资源访问——而我们实现一个用户层调度器和I/O管理器组件。通过修改硬件相关的平台层来利用VMM的资源，我们半虚拟化了FreeRTOS。图4展示了整个系统。
...
我们的基础设施以来Composite对TCaps @temp-cap，或者#emph[临时capability]的支持，它使得不同调度器之间时间的可控的委派 @temp-cap。这个基础设施使得两个我们充分利用的关键特性成为可能：（1）每个VM的调度器在它被委派的时间段内可以管理它自己的时钟中断，以及（2）如果VM独占一个硬件，则相应的硬件中断可以被安全地导向VM。请阅读@temp-cap 以了解有关如何安全地、正确地、高效地实现这一点的细节。
...
#strong[FreeRTOS CPU 半虚拟化。]FreeRTOS自身被它的设计者抽象化以支持在不同架构的系统上的可移植实现。我们修改FreeRTOS的硬件抽象层以让它作为一个VM来运行，只依赖VMM和内核服务。我们提供了某些Composite相关的用于实现调度、计时器控制以及中断的最低级的函数的实现。FreeRTOS中的线程调度直接映射到Composite 的调度，并且只需要提供给VM的对线程的capability，同时计时器是使用上文提到的一次性计时器模拟的。中断是由Composite线程提供服务，他们在`rcv`端点上挂起并响应硬件中断而激活。

== I/O虚拟化 <VI-B>
基于前文在II-C中提到的，和I/O设备之间的交互是通过位于平台定义的内存地址的映射接口进行的。由于这些设备在内存中的对齐和颗粒度阻止了通过MPU的精细的隔离，我们使用一个隔离的I/O管理器组件来调节设备访问（见图4）。这个I/O管理器被配置来允许每个VM访问设备的子集。每个VM通过对I/O管理器的受保护的组件调用来从一个设备读取数据，或者写入一个设备。我们当前把这个过程配置成静态的，更智能的preclude方式有待未来实现。

对于数据传输速率相对较低的I/O设备，数据通过Composite的同步调用，通过寄存器的方式传递。对于速率更高的设备，系统使用共享内存传输数据。如果多个VM从同一个设备读取数据，I/O管理器必须多路复用这个设备。因此设备的中断被发送给I/O管理器，然后它对订阅的VM发送软中断（通过`asnd`端点）。I/O管理器中的中断线程的优先级被设置成VM中最高的，以此接受类似@proc-aware-int 的中断。

#strong[FreeRTOS I/O 虚拟化。]FreeRTOS中的I/O流程是平台特定的，而我们提供了Composite特定的调用I/O管理器的版本。I/O引起的数据和事件被暴露给FreeRTOS队列。
...
#strong[位置无关的VM。]为了实现一定程度上的地址虚拟化，我们研究软件支持。理想化的情况是每个VM的系统组件可以被一次性加载到闪存中，然后作为库被每个VM重用，以省下闪存中的空间（$N$个VM只使用一个镜像）。我们最初探讨了通过一个VM相关的全局偏移表（GOT, Global Offset Table）来维持每个全局变量的位置的方式。一个寄存器用于引用GOT，从而使得单个镜像能被用于不同的VM之间，而不同的GOT翻译镜像内的符号。然而这个方法并不令人满意，因为（1）跨平台编译器（`gcc`）为#emph[每个]变量产生昂贵的翻译，（2）RTOS经常依照他们运行的应用在编译时期进行特殊化，这阻止了我们使用一个通用镜像，且（3）基于较大的闪存内存来看，翻译的执行代价换取省下的内存并不值得。

== VM间通信 <VI-D>
过去的研究提供了可预测的VM间通信@xen-network 。我们避免典型的将VM间通信虚拟化为网络通信。这是由于嵌入式系统中多样的网络栈和协议，也是因为不是每个VM都想（或者应该有）一个消耗内存不小的网络栈。与之相反，我们提供一个接近端到端串行通信的抽象。这使用`sinv`同步调用端点实现，并且数据是通过寄存器传输的。这个抽象的核心在于低带宽数据移动，以及VM间事件提醒。

VM间更高带宽的数据移动使用共享内存区域。这些区域按照III中探讨的MPU区域的可能的布局方案建立。I/O管理器负责建立VM之间的通信端点，以及在必要的时候，为更高带宽的数据移动建立共享内存区域。

#strong[FreeRTOS 通信半虚拟化。]至于剩下的I/O，通信整合在FreeRTOS的平台相关的代码中。来自其他VM的数据和事件被暴露为FreeRTOS队列。

= 评估 <VII>

#strong[硬件配置。]对于所有的评估，我们使用一个以216MHz运行的ARM Cortex-M7微控制器（STM32F767IGT6），拥有512KB嵌入SRAM以及1024KB嵌入闪存。微控制器有一个16KB的指令缓存和16KB的数据缓存，总是启用。同时，为了加速从嵌入闪存获取指令的速度，闪存预获取加速器总是开启的。这个微控制器同时有一个双精度FPU，也总是开启的。我们使用`gcc`编译器5.4.1版本，为所有情况使用-O2优化标志。

所有的测量都重复了10000次，并计算了平均数、最大值以及标准差。这一节的所有的条形图展示了平均值（下方的暗色条），标准差（平均值条上显示的错误条），以及极值（上方亮色条）。

#strong[软件配置。]Composite系统一直是和VMM以及I/O管理器一起运行的，系统调度器一直有足够的未分类内存来满足其他组件的请求。两个VM被加载用于测量，每个VM运行一个或者更多线程。我们使用FreeRTOS 9.0.0，评估配置了MPU和没有MPU支持的系统。

== 微基准测试 <VII-A>
#strong[硬件损耗。]很多系统操作需要和一些硬件特性，例如权限模式，MPU，以及中断进行交互。为了理解操作系统和虚拟化的损耗，我们首先探究相关操作的硬件开支。这些结果提供了一个使用他们的软件抽象的性能#emph[下界]。图5包含了模式转换、中断损耗以及MPU编程的硬件损耗。
...
MPU编程的代价包括更新 @V-A[] 中描述的MPU相关的寄存器。这个操作中的开支包括将会受影响的寄存器保存到栈里，对MPU寄存器编程，然后恢复受影响的寄存器。我们比较使用ST微电子（STM，ST Microelectronics）提供的硬件抽象层（HAL，Hardware Abstraction Layer）库来为MPU编程，以及我们的使用ARM多寄存器储存操作的优化版本。优化版本一次性编程4个寄存器，因此我们在报告中使用对4和8个区域编程的延迟。

#emph[讨论。]这些结果显示了我们用于提供隔离的硬件操作——包括MPU编程——的平均和最坏情况的延迟并不昂贵。值得注意的是，使用优化的MPU编程程序是重要的，因为STM HAL中的实现有不必要的硬件损耗。这些测量结果的方差相对较小，但是最大值有两倍甚至三倍平均值情况。这有两个原因：（1）系统保存指令和数据缓存，以及一个闪存预取器，因此在缓存未命中出现时会导致抖动，尤其是在基准测试的前几轮中，缓存没有预热时。
（2）测量是通过16位时钟模拟的64位时间戳记录的，因此当它的中断到来时就会导致损耗。因此，在实际生产环境中，这些项目的最大值几乎不会出现，并且不会像展示的那么大。

#strong[系统操作开支。]为了研究在Composite和FreeRTOS的不同配置中，几个基础的系统抽象层的代价，我们比较：（1）直接使用系统调用API的Composite组件运行，（2）#emph[没有保护设施的]FreeRTOS，从而代表轻量级RTOS的开销，（3）由FreeRTOS配置用于实现显式MPU编程支持的FreeRTOS/MPU，以及（4）在VMM和CompositeOS之上运行半虚拟化的FreeRTOS的FreeRTOS/VM。这些在图6中展示。这些系统提供的核心操作包括中断处理、线程上下文切换，以及线程间通信（我们称之为IPC）。

所有这些系统将中断执行分割为“上半部分”中断服务过程（ISR, Interrupt Service Routine），和“下半部分”也就是在一个线程上下文中的执行。中断延迟由从ISR激活到对应的中断处理线程开始运行的时间间隔来测量。在Composite中，这是通过激活一个ISR中的内核“异步发送”（`asnd`）端点，然后用它激活一个阻塞等待在“接收”`rcv`端点上的线程。在FreeRTOS里，做法是相似的：ISR会使用`xQueueSendFromISR()`来发送到一个队列里，从而激活中断线程。在所有情况下，进入ISR和开始中断进程执行的间隔就是延迟。在FreeRTOS/VM中，这个延迟是用从Composite的硬件ISR执行，到一个半虚拟化的FreeRTOS线程从它的中断队列收到中断的间隔测量的。

#emph[讨论。]在很多情况下，FreeRTOS表示一个我们期望的更低的开支下界。它包含不受保护的设施，并且包含线程分发在内的一些过程的高度优化的版本。我们至少期待图5中的任何一个使用了基于MPU的保护的系统出现额外的延迟。结果显示Composite的操作大体上和FreeRTOS/MPU相近，除了线程上下文切换的代价。Composite和FreeRTOS/MPU中的上下文切换都显示出多于FreeRTOS的开支，这是因为系统调用和MPU编程的开支。FreeRTOS/VM同时显示出来自FreeRTOS（由于它的调度逻辑）#emph[和]Composite（它进行线程上下文切换）的开支。
...
#strong[Composite 微基准测试。]为了更好地理解这些结果，我们在基础Composite系统上进行了一些测量（图8）。在这一部分，我们讨论了下列Composite的操作：（1）组件内线程切换，（2）组件间线程切换，（3）同步调用进入时间，（4）同步调用返回时间，（5）中断延迟，（6）组件内异步通信时间，以及（7）组件间异步通信时间。
...
在Composite里有两种ICP机制：通过线程迁移[20]的同步调用（`sinv/sret`），以及异步发送/接收（`asnd/rcv`）。两个组件之间的同步调用涉及组件之间的切换（包括MPU编程和系统调用），是最常见的内存隔离的组件之间的通信机制。`snd1`测量一个`asnd`和一个在`rcv`上等待的线程的激活之间的延迟。相比之下，`snd2`是在不同组件的线程之间，因此需要MPU编程。中断延迟`int`是从图6中复制的，展示了对中断延迟是与异步通信高度相关的这一直觉期望。这并不出人意料，因为这两个操作共享相同的抽象和代码。
...
我们集中在对虚拟化的两个方面的评估。首先，`vect2vm`是从Composite内核中断向量/ISR执行到VM中的FreeRTOS的ISR执行的时间。其次，`vect2thd`是从Composite ISR，到FreeRTOS的接收线程执行的时间。我们可以从这个结果看出，大多数开支来自FreeRTOS#emph[内部]对这个中断的传递。这验证了我们先前的分析：当前的调度器实现总是在处理一个中断之后切换回一个“调度器线程”，并且导致了巨大的上下文切换代价。我们将在未来的工作中优化这一部分，但是正如我们在图7中所示，当前的代价对于许多I/O形式都是合理的。
...
#emph[讨论。]这些结果确认了在图6中看到的虚拟化环境的中断开支的来源主要是由于过度的线程上下文切换。`qsnd`和`isnd`通信机制之间的区别相当有趣：VM间通信比基于FreeRTOS API的VM内通信快很多。注意到`isnd`是一个“来回”测量，而`qsnd`是一个“单程”测量。因此，我们的VM间通信是比VM内通信快两倍多。这大部分是因为VM间通信对优化的Composite IPC机制的直接依赖。

== 半虚拟化复杂度 <VII-C>

半虚拟化依赖对被虚拟化操作系统的最底层的修改。在FreeRTOS/VM中，我们把FreeRTOS和平台相关的层移植到使用Composite的调度支持，计时器支持，以及I/O（通过对I/O管理器组件的调用）。这个平台实现有363行源代码（SLOC, Source Lines of Code）。相比之下，我们的微控制器的裸机平台代码，无MPU版为483 SLOC，有MPU支持版为622 SLOC。尽管源代码行数不是一个完美的复杂度度量标准，我们还是得出结论，在Composite中半虚拟化FreeRTOS并不算大工作量，并且我们期望其他RTOS也可以被移植到这个系统。
...
#strong[内核和虚拟化开支。]表1展示了每个软件组件的大小。内核和VMM组件增加了一个不变的内存开支，而FreeRTOS中的改变带来的开支随着VM的数量增加。半虚拟化的FreeRTOS消耗更多的RAM，这主要是因为Composite的用户级调度库静态分配所有线程；然而，它的代码大小缩小了，因为架构相关的代码被高效地移动到Composite内核里了。FreeRTOS进行系统资源（包括线程和队列）的动态分配，所以这个表并不是一个对内存需求的完美展示，因为FreeRTOS会在运行时需求更多。我们相信这些开支对大多数系统都是合理的。

#strong[内存分配算法的高效性。]

静态内存布局是使用@III[]中尝试尽力利用可用的MPU区域和子区域的算法确定的。然而，MPU对齐和大小限制严重影响了M这些布局，从而产生了内部碎片。

为了评估我们系统的内存开支，我们测试5个不同的VM配置，使用3种不同的共享内存配置。这些配置都包含运行MiBench@mibench 应用的VM，并有如表3所示的内存限制。每个#emph[小（small）]VM都包含一个MiBench应用。每个#emph[大（big）]VM都包含所有5个内存大小各异的MiBench应用。我们的5个不同的应用配置如下：（1）3小（`basicmath`，`dijkstra`，`gsm`），（2）3小1大（`basicmath`，`dijkstra`，`gsm`，`big`），（3）4小1大（`basicmath`，`dijkstra`，`gsm`，`pbmsrch`，`big`），（4）5小（`basicmath`，`dijkstra`，`gsm`，`pbmsrch`，`rijndael`），以及（5）5小1大（`basicmath`，`dijkstra`，`gsm`，`pbmsrch`，`rijndael`，`big`）。这些配置中的每个都同时包含内核和VMM组件。每个VM被编译成应用和半虚拟化的FreeRTOS。
...
为了评估每个配置中的开支，我们使用内存布局算法。我们给系统一个可配置的SRAM大小，然后使用逐渐减少的内存运行启发式算法，直到当给定$X K B$时它找不到一个解。每个配置使用的全部内存为$Y K B$，表2包含了内存开支（$(X-Y)/Y$）。
...
#strong[VM可扩展性。]随着对隔离需求的增加，一个单一系统将会需要多个VM。为了评估VM数量增长的系统的可扩展性，我们考虑每个VM的大小，以及必要的MPU对齐参数的变化。每个FreeRTOS/VM（没有应用）可以放进32KB的内存块。如果所有的虚拟机都是空的，那么STM32F767IGT6（512KB SRAM）可以放8个，同时包含内核和VMM组件，剩下128KB用于共享映射和动态分配。这表示一个仅基于系统开支的限制，而更大或更需要计算的应用实际上会导致更少的VM。即使对于现有的系统，如果一个系统有大量的应用并且希望将他们隔离到不同的VM中，这也是有用的。不论怎样，对这么小的系统来说，这仍然是令人惊讶的可扩展性。

为了理解#emph[带有]应用的系统的可扩展性，表3包含了和半虚拟化的FreeRTOS一起编译的不同的MiBench@mibench 应用的内存需求。在实际的配置中，VM内的应用和VM的内存占用相近或者更小。因此，一个中等的微控制器可以轻松的容纳4个VM，而有更多SRAM（1MB或者更多）的高端微控制器可以轻松容下12个或者更多VM。

= 相关工作 <VIII>

#strong[避免硬件保护。]许多内存有限的系统通过语言安全性提供隔离。这些包括TinyOS @nesc 和 Tock @multiprog-64kb。这些方式有个好处：软件bug可以通过软件检查来限制，这可以减少许多操作的开支。然而，这使得分离的代码的虚拟化更加困难。其他项目在微控制器上使用安全的语言，但是强调以性能和可预测性的代价换取可编程性，@micropython，@elua，@microej，@mjs。
...
有些系统将保护域交互（IPC）简化成函数调用以追求性能，并且使用过程，或者协作调度 @dynamic-os，@nesc，@multiprog-64kb。这个设计决定在所有的代码都有很大的期望能够遵循低且有限的运行时间，否则这个并不抢占的性质会阻止临时的隔离。
...
#strong[虚拟化支持。]据我们所知，这篇文章提出了第一个基于MPU的微控制器的虚拟化基础设施。和我们的I/O虚拟化类似，@io-virt 实现了一个处理I/O的FreeRTOS任务，而其他任务将他们的I/O访问导向它。相比之下，我们的研究注重一个实现CPU、内存和I/O三者全部的虚拟化基础设施。
...
#image("../../1-pan18mpu.pdf")

#raw(read("./thesis.typ"), lang: "typ")
